...
import { getLessonData, PHASE_1_LESSONS } from '@/lib/backend-course';
...
export function generateStaticParams() {
  return PHASE_1_LESSONS.map((l) => ({ lesson: l.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { lesson: slug } = await params;
  const meta = PHASE_1_LESSONS.find((l) => l.slug === slug);
  if (!meta) return {};
  return {
...
export default async function LessonPage({ params }: Props) {
  ...
  return (
    <div className="lesson-page-wrapper">
      ...
      {/* Bottom navigation (React-native, replaces the lesson's own nav) */}
      <nav className="lesson-bottom-nav">
        <div className="lesson-bottom-nav-inner">
          {data.prev ? (
            <Link href={`${COURSE_BASE}/phase-1/${data.prev.slug}`} className="lesson-nav-link">
          ...
          {data.next ? (
            <Link href={`${COURSE_BASE}/phase-1/${data.next.slug}`} className="lesson-nav-link lesson-nav-link--next">
            ...
            <div className="lesson-nav-link lesson-nav-link--next lesson-nav-link--complete">
              <span className="lesson-nav-dir">✓ Phase 1 Complete</span>
              <span className="lesson-nav-title">More phases coming soon</span>
            </div>
          )}
        </div>
      </nav>
...
// Shared CSS for the lesson reading experience.
// Scoped to .lesson-host so it doesn't leak to the site's own styles.
const SHARED_COURSE_CSS = `
/* ── Course reset — scope all lesson styles inside .lesson-host ── */
.lesson-host {
  --bg: #0f1117;
  --surface: #1a1d27;
  --surface2: #22263a;
...
/* Suppress the lesson's built-in nav — we render our own below */
