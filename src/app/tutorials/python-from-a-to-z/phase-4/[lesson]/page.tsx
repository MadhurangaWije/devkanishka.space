...
const PHASE = PHASES.find((p) => p.number === 4)!;
...
export default async function LessonPage({ params }: Props) {
  ...
  let data;
  try {
    data = getLessonData(4, slug);
  } catch {
    notFound();
  }

  const prevPhase = PHASES.find((p) => p.number === 3)!;
  const nextPhase = PHASES.find((p) => p.number === 5)!;
      ...
      <nav className="lesson-bottom-nav">
        <div className="lesson-bottom-nav-inner">
          {data.prev ? (
            ...
            // First lesson in Phase 4 — link back to last lesson of Phase 3
            <Link
              href={`${COURSE_BASE}/${prevPhase.urlSegment}/${prevPhase.lessons[prevPhase.lessons.length - 1].slug}`}
              className="lesson-nav-link"
            >
              <span className="lesson-nav-dir">← Phase 3</span>
          ...
          {data.next ? (
            ...
            // Last lesson in Phase 4 — link forward to first lesson of Phase 5
            <Link
              href={`${COURSE_BASE}/${nextPhase.urlSegment}/${nextPhase.lessons[0].slug}`}
              className="lesson-nav-link lesson-nav-link--next"
            >
              <span className="lesson-nav-dir">Next → Phase 5</span>
