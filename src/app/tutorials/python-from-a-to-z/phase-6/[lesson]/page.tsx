...
const PHASE = PHASES.find((p) => p.number === 6)!;
...
export default async function LessonPage({ params }: Props) {
  ...
  let data;
  try {
    data = getLessonData(6, slug);
  } catch {
    notFound();
  }

  const prevPhase = PHASES.find((p) => p.number === 5)!;
      ...
      <nav className="lesson-bottom-nav">
        <div className="lesson-bottom-nav-inner">
          {data.prev ? (
            ...
            // First lesson in Phase 6 — link back to last lesson of Phase 5
            <Link
              href={`${COURSE_BASE}/${prevPhase.urlSegment}/${prevPhase.lessons[prevPhase.lessons.length - 1].slug}`}
              className="lesson-nav-link"
            >
              <span className="lesson-nav-dir">← Phase 5</span>
          ...
          {data.next ? (
            ...
            <div className="lesson-nav-link lesson-nav-link--next lesson-nav-link--complete">
              <span className="lesson-nav-dir">✓ Course complete</span>
              <span className="lesson-nav-title">All {totalLessons} lessons done</span>
            </div>
          )}
        </div>
      </nav>
