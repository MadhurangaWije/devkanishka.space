...
export default function DockerCourseOverviewPage() {
  ...
  const totalLessons = DOCKER_PHASES.reduce((s, p) => s + p.lessons.length, 0);
  ...
  return (
    <div className="px-6 md:px-12 py-12 max-w-3xl">
      ...
      <div className="mb-10">
        <div className="flex flex-wrap items-center gap-3 mb-4">
          ...
          <span className="font-mono text-xs text-text-muted">
            {totalLessons} lessons · {DOCKER_PHASES.length} phases
          </span>
        </div>
      ...
      <div className="mt-10">
        <Link
          href={`${COURSE_BASE}/${DOCKER_PHASES[0].urlSegment}/${DOCKER_PHASES[0].lessons[0].slug}`}
